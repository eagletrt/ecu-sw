...
EAGLETRT_STATIC struct CanCommunicationNetworkConfig test_valid_configs[CAN_COMM_NET_COUNT];
...
void setUp(void) {
    ...
    for (size_t i = 0; i < CAN_COMM_NET_COUNT; ++i) {
...
void test_can_communication_api_init_should_fail_on_null_config(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(CAN_COMM_RC_NULL_POINTER, rc, "Module initialization did not fail when passed a completely NULL configuration array");
}

void test_can_communication_api_init_should_fail_on_missing_mandatory_send_callback(void) {
    memset(&handler, 0, sizeof(handler));

    struct CanCommunicationNetworkConfig invalid_configs[CAN_COMM_NET_COUNT] = {
        [CAN_COMM_NET_PRIMARY] = { .send = NULL, .on_receive = mock_application_on_receive },
        [CAN_COMM_NET_SECONDARY] = { .send = mock_can_send, .on_receive = mock_application_on_receive },
        [CAN_COMM_NET_INVERTER] = { .send = mock_can_send, .on_receive = mock_application_on_receive }
    };

    enum CanCommunicationReturnCode rc = can_communication_api_init(invalid_configs);

    TEST_ASSERT_EQUAL_MESSAGE(CAN_COMM_RC_NULL_POINTER, rc, "Module initialization bypassed safety checks when mandatory driver send callback was unassigned");
}
...
void test_can_communication_api_init_should_fail_on_missing_mandatory_receive_callback(void) {
    ...
    struct CanCommunicationNetworkConfig invalid_configs[CAN_COMM_NET_COUNT] = {
        [CAN_COMM_NET_PRIMARY] = { .send = mock_can_send, .on_receive = mock_application_on_receive },
        [CAN_COMM_NET_SECONDARY] = { .send = mock_can_send, .on_receive = NULL },
        [CAN_COMM_NET_INVERTER] = { .send = mock_can_send, .on_receive = mock_application_on_receive }
    };
...
void test_can_communication_api_add_to_rx_should_fail_on_invalid_network_bounds(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(CAN_COMM_RC_INVALID_NETWORK, rc, "Module accepted an insertion onto a completely out-of-bounds network segment parameter index");
}

void test_can_communication_api_add_to_rx_should_fail_with_queue_full(void) {
    enum CanCommunicationReturnCode nominal_rcs[CAN_COMM_RX_QUEUE_CAPACITY];
    enum CanCommunicationReturnCode saturation_rc;
    struct CanCommunicationFrame frame = { .id = 0x123U, .length = 8U, .data = { 1, 2, 3, 4, 5, 6, 7, 8 } };

    for (uint32_t i = 0; i < CAN_COMM_RX_QUEUE_CAPACITY; i++) {
    ...
    TEST_ASSERT_EACH_EQUAL_INT_MESSAGE(CAN_COMM_RC_OK, nominal_rcs, CAN_COMM_RX_QUEUE_CAPACITY, "Queue insertion failed unexpectedly prior to hitting nominal tracking capacity milestones");
    TEST_ASSERT_EQUAL_MESSAGE(CAN_COMM_RC_QUEUE_FULL, saturation_rc, "Module failed to return a saturation code error after memory layout tracks reached complete capacity limits");
}
/*! \} */
...
void test_can_communication_api_process_rx_should_fail_on_invalid_network_bounds(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(CAN_COMM_RC_INVALID_NETWORK, rc, "Processing cycle bypass bounds verification rules when passed an out-of-bounds network context index");
}
...
void test_can_communication_api_process_rx_should_forward_receive_handler_errors(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(CAN_COMM_RC_RECEIVE_HANDLER_ERROR, process_rc, "The processing loop did not capture or bubble up unexpected handler failures occurring within application routines");
}
/*! \} */
...
void test_can_communication_api_add_to_tx_should_fail_on_invalid_network_bounds(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(CAN_COMM_RC_INVALID_NETWORK, rc, "Transmission tracking line failed to intercept an out-of-bounds network identity selection");
}
...
void test_can_communication_api_add_to_tx_should_fail_with_queue_full(void) {
    enum CanCommunicationReturnCode nominal_rcs[CAN_COMM_TX_QUEUE_CAPACITY];
    enum CanCommunicationReturnCode saturation_rc;
    struct CanCommunicationFrame frame = { .id = 0x50U, .length = 1U, .data = { 0xFF } };

    for (uint32_t i = 0; i < CAN_COMM_TX_QUEUE_CAPACITY; i++) {
    ...
    TEST_ASSERT_EACH_EQUAL_INT_MESSAGE(CAN_COMM_RC_OK, nominal_rcs, CAN_COMM_TX_QUEUE_CAPACITY, "Transmission queue ring rejected allocation insertions before hitting true limits");
    TEST_ASSERT_EQUAL_MESSAGE(CAN_COMM_RC_QUEUE_FULL, saturation_rc, "Transmission pipeline failed to report queue saturation code configurations upon limits overflow");
}
/*! \} */
...
void test_can_communication_api_process_tx_should_fail_on_invalid_network_bounds(void) {
    enum CanCommunicationReturnCode rc = can_communications_api_process_tx(CAN_COMM_NET_COUNT);

    TEST_ASSERT_EQUAL_MESSAGE(CAN_COMM_RC_INVALID_NETWORK, rc, "Transmission pipeline processing accepted out-of-bounds context validation configurations");
}
...
void test_can_communication_api_process_tx_should_flush_to_hardware(void) {
    ...
    process_rc = can_communications_api_process_tx(CAN_COMM_NET_INVERTER);
